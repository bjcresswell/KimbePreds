...
abunplotresults <- 
  ...
  ylab('Abundance (n)\n')+
  xlab(NULL)+
  geom_linerange(aes(ymin = Mean - CI95 , ymax = Mean + CI95),  size = 0.3)+
...
abunplotcontrasts <- 
  ...
  geom_linerange(aes(ymin = contrastlowerCI95,  ymax = contrastupperCI95), size = 0.3) +
  geom_linerange(aes(ymin = contrastlowerCI66,  ymax = contrastupperCI66), size = 0.6) +
...
right.panel <-
  ...
  filter(Reeftype == 'P') %>% 
  ggplot() +
  ylab("Contrast effect size estimate (±95/66% CI)")

mainfig1 <- ggpubr::ggarrange(mainfig, right.panel, ncol = 2, widths = c(1, 0.05) )
...
mainfig2 <- ggpubr::ggarrange(mainfig)
mainfig2 <- annotate_figure(mainfig2, right = text_grob("Contrast effect size estimate (±95/66% CI)", rot = 90))
