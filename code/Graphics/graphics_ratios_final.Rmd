...
Notes on aesthetics for Coral Reefs
...
load("../../data/abunratios.RData")

# Tweak labels so figure looks nice
abunplotdata <- 
abunplotdata %>% 
  mutate(Reeftype = dplyr::recode(Reeftype,
                                         `Pinnacle` = 'P',
                                         `Offshore` = 'O',
                                         `Nearshore` = 'N'))
...
abunratios <- 
  abunratios %>% 
  mutate(Contrast = recode_factor(contrasts.contrast,
...
abunplotcontrasts <- 
  ggplot(abunratios, aes(y=contrasts.ratio,  x=Contrast)) +
  geom_linerange(aes(ymin = contrasts.lower.CL,  ymax = contrasts.upper.CL), size = 0.5) +
  ...
  theme(axis.text.y = element_text(family="Arial", size= 10, colour = 'black'))+
  theme(axis.line = element_blank(), panel.border = element_rect(size = 1, fill = "transparent")) +
  annotate("text", x = 3.4, y = 10, label = "d")
...
# 2. Using ggpubr::annotate_figure (requires passing the mainfig to ggarrange first anyways as it doesn't work with the plot 'as-is')
mainfig2 <- 
  ggpubr::ggarrange(mainfig) +
  annotate_figure(mainfig, right = text_grob("Effect size (+ 95 CI)", vjust = 2.2, rot = 270, family = 'Arial', size = 10))
...
ggsave(mainfig1, filename= '../../output/rfigures/main_fig1.tiff',  width = 174, height = 107, units = "mm", dpi = 600)
 ggsave(mainfig2, filename= '../../output/rfigures/main_fig2.tiff',  width = 174, height = 107, units = "mm", dpi = 600)
