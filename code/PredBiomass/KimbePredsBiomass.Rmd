...
rtbar <- ggplot(predreeftypestats, aes(x=Reeftype, y=mean))+
  ...
  scale_y_continuous("Mean predator count (±SE)")+
...
# Fit models
Will start with basic GLMM with site as random factor. Definitely not going to be Gaussian, Gamma (zeros), Beta (not 0-1). Will start with Poisson, Neg Binom, ZI and Tweedie. Will introduce offset later on to account for total fish biomass
...
```
- Neg binom or Tweedie best residual fit

# Check using binned residuals:
```{r biom-binresid}
binned_residuals(biom.glmmp) #100% within error bounds
binned_residuals(biom.glmmnb2) #100% within error bounds
binned_residuals(biom.glmmzi) #100% within error bounds
binned_residuals(biom.glmmt) #100% within error bounds
```


# Validation with AIC
```{r biom-aic1}
AIC(biom.glmmp, biom.glmmnb2, biom.glmmzi, biom.glmmt)
```

NB best fit so will work with this 

# Model tweaking
Going to add the offset straight away
```{r biom-nb-offset}
biom.glmmnboff <- update(biom.glmmnb2, ~ . + offset(log(Transect_fish_biomass_kg)))
```

# Revalidation with AIC
```{r biom-aic2}
AIC(biom.glmmnboff, biom.glmmnb2)
#binned_residuals(biom.glmmnboff)
```
...
biomcontrasts <- emmeans(biom.glmmnboff, pairwise ~ Reeftype, transform='response', combine=TRUE) %>%
  confint() %>% 
  as.data.frame() 

# pvalues
biompvals <- emmeans(biom.glmmnboff,  ~ Reeftype, transform='response', combine=TRUE) %>% 
  pairs() %>% 
  as.data.frame() 
  
# Combine for full results table
biomresults <- bind_cols(biomcontrasts, biompvals[5:6]) 
...
# Ratios with CI
biomratios <- emmeans(biom.glmmnboff, pairwise ~ Reeftype, type='response', combine=TRUE) %>%
  confint() %>% 
  as.data.frame()

# p values
biomratiopvals <- emmeans(biom.glmmnboff,  ~ Reeftype, type='response', combine=TRUE) %>% 
  pairs() %>% 
  as.data.frame() 

# Combine
biomratios <- bind_cols(biomratios, biomratiopvals[6:7]) 
...
biomratios
```
Slightly different story with the ratios - don't know why!!  The story is pretty much the same however.

# Create plot data
Final step is to create the df to be used for plot making 
```{r biom-plot-data}
biomplotdata <- emmeans(biom.glmmnboff, pairwise ~ Reeftype, transform='response') %>%
  confint() %>% 
  as_tibble() 
