...
biomcontrasts <- emmeans(biom.glmmtoff, pairwise ~ Reeftype, transform='response', combine=TRUE) %>%
  confint() %>% 
  as.data.frame() 

# pvalues
biompvals <- emmeans(biom.glmmtoff,  ~ Reeftype, transform='response', combine=TRUE) %>% 
  pairs() %>% 
  as.data.frame() 
...
```{r biom-plot-data}
biomplotdata <- emmeans(biom.glmmtoff, pairwise ~ Reeftype, transform='response') %>%
  confint() %>% 
  as_tibble() 
