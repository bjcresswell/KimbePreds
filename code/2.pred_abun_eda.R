...
### Script for pulling out predator statistics by reeftype/site/survey (for percent analysis see separate script)
...
# On raw predator abundance (this would be the preferred response variable to measure)
ggplot(predsum, aes(x=No_PredsZ)) + 
  geom_histogram(binwidth=50, color="black", fill="grey")

# On predator abundance + 1
ggplot(predsum, aes(x=No_Preds)) + 
  geom_histogram(binwidth=50, color="black", fill="grey")

# Clearly this is count data with a lot of zeros, so we'll most likely need a Poisson or neg binom model fit for either of these

# On proportion of assemblage that are predators +0.00000001
ggplot(predsum, aes(x=PredPC)) + 
  geom_histogram(binwidth=0.01, color="black", fill="grey") 

# On raw predator proportions
ggplot(predsum, aes(x=PredPCZ)) + 
  geom_histogram(binwidth=0.01, color="black", fill="grey") 

# Again a lot of zeros but it's percentage/proportion data so likely will need a beta distribution for this
...
ssbox <- ggplot(predsum, aes(x=SiteCode, y=No_Preds, fill=SurvCode))+
  geom_boxplot()+
  scale_y_continuous(limits = c(0,400), "Median predator count (±SE)") +
  scale_x_discrete("Site and survey period")+
  scale_fill_viridis_d()+
  theme_scientific()+
  theme(legend.key.size = unit(0.4, "cm"))+
...
rtbox <- ggplot(predsitestats, aes(x=Reeftype, y=mean))+
  geom_boxplot()+
  scale_y_continuous(limits = c(0,100), "Mean predator count (±SE)") +
  #geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=.2, position=position_dodge(.9))+
  scale_x_discrete("Site and survey period")+
  scale_fill_viridis_d()+
  theme_scientific()+
