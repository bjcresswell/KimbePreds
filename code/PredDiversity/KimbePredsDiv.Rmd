...
getwd()
source("code/PredDiversity/pred_div_diw.R", local = knitr::knit_global())
```
...
# Combine
richratios <- bind_cols(richratios, richratiospvals[5:6]) 
...
```
-> Slightly under-dispersed - non-significant
...
```{r shan-emmeans}
emmeans(pshan.glmmT, ~Reeftype, type="response") %>% 
  #confint() #%>% 
  as.tibble() #%>% 
...
pshan.glmmT %>% 
  emmeans(pairwise ~ Reeftype, type='response') %>%
  confint() %>% 
  as.data.frame() 
...
shanplotdata %>% 
  ...
  bind_cols(shanratios[c(8,11,12)]) %>% 
