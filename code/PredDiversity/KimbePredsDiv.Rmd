...
```
Fixed effects a better fit than R.E. only and poisson best fit overall
...
pshan.glmm <- glmmTMB(Shannon ~ Reeftype+(1|Site), data = preddiv, family=gaussian(link = 'identity'), ziformula = ~0,  REML = TRUE)
...
shanplotdata <- emmeans(pshan.glmmT, pairwise ~ Reeftype, transform='response') %>%
  confint() %>% 
  as_tibble() 
