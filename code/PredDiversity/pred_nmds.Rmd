...
pred_nmds_plot <-
  ...
  geom_text(data = species.labels, aes(coord_1, coord_2, label = label), size = 5, fontface = "italic", ) + # Text for these           
  geom_segment(data = species.scores, aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2),              # And arrows
               arrow = arrow(length = unit(0.2, 'cm'), type = "closed"), colour = "black", arrow.fill = "black", alpha = 0.8) +
...
pred_nmds_plot + 
  ...
  annotation_custom(
    ggplotGrob(predvenn), 
    xmin = -18.5, xmax =0.5, ymin = -1, ymax = 3)
