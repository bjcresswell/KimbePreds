...
### And plot
```{r mds-plot1}
theme_set(theme_classic(base_family = "Arial", base_size = 5))
...
# Coords for labels
```{r}
Coord_1 <- c(-1.6,	-1.5,	  -6.3,	 -3.1,	   -1.1,   -0.5)
Coord_2 <- c(0.5,	 -0.3,	 -0.6,    0.2,	  1.0,	 -0.2)
```

# Coords to go with species names
```{r}
#Coord_1 <- c(-1.55,	-1.9,	  -6.1,	  -4.8,	 -1.2,   0.6)
#Coord_2 <- c(0.5,	 -0.5,	 -0.8,    0.3,	  1.0,	 -0.2)
```
...
# Plot for exit seminar

```{r fig.height = 87/25.4, fig.width = 140/25.4}
pred_nmds_plot <-
  ...
  stat_ellipse(geom = "polygon", aes(colour = grp, fill = grp), alpha=0.2, level = 0.95, type='t',size =0.95, show.legend = FALSE) +          # 95% conf ellipses
  geom_point(aes(colour = grp, fill = grp, shape = grp), alpha=0.8, size = 3) +  # or could even include scale by Shannon score
  xlim(-7, 3)+
  ylim(-3, 3)+
...
```{r}
ggsave(pred_nmds_plot, filename= 'output/rfigures/pce_figs/prednmds_pce.pdf', device = cairo_pdf,  width = 140, height = 87, units = "mm", dpi = 300)
```
