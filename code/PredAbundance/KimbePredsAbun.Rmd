...
# Tweedie
pred.glmmt <- glmmTMB(No_Preds~Reeftype+(1|SiteCode), data=predsum, family=tweedie())  
```
...
emmeans(pred.glmmnb, pairwise~Reeftype, transform="response") %>% 
  confint()
```
-> So emmeans suggest that pinnacles have more predators than both offshore emergent and near shore reefs, but that this is highly variable.  What about if we take into account overall numbers of fishes?


# Model tweaking
Now going to add the offset (against total numers of fishes in assemblage)
...
```{r abun-aic2}
AIC(pred.glmmnb,  pred.glmmnboff)
```
...
# R2 if you want but not very useful for glmm
#library(performance)
#performance::r2(pred.glmmnboff)

# emmeans and contrasts first
abuncontrasts <- emmeans(pred.glmmnboff, pairwise ~ Reeftype, transform='response', combine=TRUE) %>%
  confint() %>% 
  as.data.frame() 

# pvalues
abunpvals <- emmeans(pred.glmmnboff,  ~ Reeftype, transform='response', combine=TRUE) %>% 
  pairs() %>% 
  as.data.frame() 
  
# Combine for full results table
abunresults <- bind_cols(abuncontrasts, abunpvals[5:6]) 
...
# Can also check ratios (using type="response"):
```{r abun-emmeans-ratios}

# Ratios with CI
abunratios <- emmeans(pred.glmmnboff, pairwise ~ Reeftype, type='response', combine=TRUE) %>%
  confint() %>% 
  as.data.frame()
...
# Combine
abunratios <- bind_cols(abunratios, abunratiopvals[6:7]) 
...
## So, taking into account overall fish abundance, pinnacles have significanly more predators on them than either offshore or near shore (11 more per 150m^2 transect)  (12 more per 150m^2 transect) reefs, but this is highly variable (confints)
...
abunplotmeans <- emmeans(pred.glmmnboff, pairwise ~ Reeftype, transform='response') %>% 
  ...
  transmute(Reeftype = emmeans$Reeftype,
            ...
            `95CI` = emmeans$response-emmeans$lower.CL)

# Then the 66% confidence intervals
CI66 <- emmeans(pred.glmmnboff, pairwise ~ Reeftype, transform='response') %>%
  confint(level = 0.66) %>% 
  as_tibble() %>% 
...
abunplotcontrasts <- emmeans(pred.glmmnboff, pairwise ~ Reeftype, transform='response') %>%
  confint() %>% 
  as_tibble() %>% 
  transmute(
...
# Run this chunk to execute whole script without procucing html output
```
